...
// Inisialisasi variabel pesan untuk menghindari pesan tidak terdefinisi
$pesan = "";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validasi input (pastikan $id adalah bilangan bulat)
    if (!is_numeric($id)) {
        $pesan = "ID harus berupa bilangan bulat";
    } else {
        // Update data anggota
        $query = "UPDATE pengguna1 SET nama=?, foto=?, email=?, tlp=?, status=?, level=? WHERE id=?";
        $stmt = mysqli_prepare($koneksi, $query);
        
        if ($stmt) {
            // Use "sssssis" for the type definition string
            mysqli_stmt_bind_param($stmt, "sssssis", $nama, $foto, $email, $tlp, $status, $level, $id);
            ...
            if ($updateResult) {
                ...
                exit(); // Make sure to exit after redirecting to another page
            } else {
            ...
            $pesan = "Gagal membuat statement: " . mysqli_error($koneksi);
        }
    }        
}

// Ambil data anggota yang akan di-edit
if (isset($_GET['id'])) {
    $id_to_edit = $_GET['id'];

    // Validasi input (pastikan $id_to_edit adalah bilangan bulat)
    if (!is_numeric($id_to_edit)) {
        $pesan = "ID yang akan di-edit harus berupa bilangan bulat";
    } else {
        $query = "SELECT * FROM pengguna1 WHERE id=?";
        $stmt = mysqli_prepare($koneksi, $query);

        if ($stmt) {
            // Gunakan "i" untuk id sebagai parameter tipe integer
            mysqli_stmt_bind_param($stmt, "i", $id_to_edit);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_bind_result($stmt, $id, $nama, $foto, $email, $tlp, $status, $level);
        ...
        <div class="sidebar-brand-text mx-3"> <sup></sup></div>
            ...
            <li class="nav-item">
                ...
                <div id="collapseUtilities" class="collapse" aria-labelledby="headingUtilities"
                    ...
                    <div class="bg-white py-2 collapse-inner rounded">
                        ...
                        <a class="collapse-item" href="utilities-color.html">Peminjaman Buku</a>
                        <a class="collapse-item" href="utilities-border.html">Pengembalian Buku</a>
                    </div>
                </div>
            </li> 
            <li class="nav-item">
            ...
            <li class="nav-item">
                ...
                <div id="collapsePages" class="collapse" aria-labelledby="headingPages" data-parent="#accordionSidebar">
                    <div class="bg-white py-2 collapse-inner rounded">
                        <a class="collapse-item" href="login.php">Laporan Peminjaman</a>
                        <a class="collapse-item" href="registrasi.php">Laporan Pengembalian</a>
                    </div>
                </div>
            </li>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <div class="form-group">
                                ...
                                <input type="text" name="nama" class="form-control" value="<?php echo isset($nama) ? $nama : ''; ?>">
                            </div>
                            <label for="foto" class="fw-bold">Gambar</label>
                            <input type="file" name="foto"  class="form-control" value="<?php echo isset($foto) ? $foto : ''; ?>">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="tlp" class="form-control" value="<?php echo isset($tlp) ? $tlp : ''; ?>">
                            </div>
                            <div class="form-group">
                                <label for="status">Status</label>
                                <input type="text" name="status" class="form-control" value="<?php echo isset($status) ? $status : ''; ?>">
                            </div>
                            <div class="form-group">
                                <label for="level">Level</label>
                                <input type="text" name="level" class="form-control" value="<?php echo isset($level) ? $level : ''; ?>">
                            </div>
                           
                            <!-- Menambahkan input hidden untuk menyimpan ID rak yang akan diubah -->
