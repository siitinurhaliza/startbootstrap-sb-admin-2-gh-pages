...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $key = mysqli_real_escape_string($koneksi, $_POST['key']); // Sanitize the input to prevent SQL injection

    // Modify your SQL query to include the search condition
    $query = "SELECT * FROM anggota WHERE poto LIKE '%$key%' OR nisn LIKE '%$key%' OR nama LIKE '%$key%' OR kelas LIKE '%$key%' OR jk LIKE '%$key%' OR alamat LIKE '%$key%'";
    $datas = mysqli_query($koneksi, $query) or die(mysqli_error($koneksi));
} else {
    // If the form is not submitted, fetch all data
    $datas = mysqli_query($koneksi, "SELECT * FROM anggota") or die(mysqli_error($koneksi));
}
        ...
        <div class="col-12">
            ...
            <div class="card mt-3">
                <div class="card-header">
                    <div class="float-left"> <!-- Add a container for left-aligned content -->
                        Data Anggota
                    </div>
                ...
                <div class="card-body">
                    <form action="" method="post">
                        <div class="row mb-3">
                            ...
                            <a href="tambahanggota.php" class="btn btn-primary mb-4"><i class="bi bi-bookmark-plus-fill">TAMBAH DATA</i>
     ...
     // Redirect to the main page after deletion
                            ...
                            if (mysqli_num_rows($datas) > 0) {
                                while ($row = mysqli_fetch_assoc($datas)) {
                                    $rowColor = ($no % 2 == 0) ? '#f9f9f9' : '#e0e0e0'; // Set alternating row color
                            ?>
                                    <tr style="background-color: <?= $rowColor; ?>">
                                        <td><?= $no; ?></td>
                                        <td><?= $row['nama']; ?></td>
                                        <td style="text-align: center;"><img src="img/<?php echo $row['poto']; ?>" style="width: 120px;"></td>
                                        <td><?= $row['nisn']; ?></td>
                                        ...
                                        <div class="btn-group" role="group">
                                            <a href="editanggota.php?id=<?= $row['id']; ?>" class="btn btn-sm btn-warning">Edit</a>
                                            <div class="btn-group" role="group">
                                            <a href="?id=<?= $row['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Anda yakin ingin hapus data ini?');">Hapus</a>
                                            <a href="cetak_kartu_pustaka.php?id=<?= $row['id']; ?>" class="btn btn-sm btn-info">Cetak Kartu Pustaka</a>
                                        </div>
                                    </td>
