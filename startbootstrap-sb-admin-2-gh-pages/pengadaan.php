...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   // Ubah kueri SQL Anda untuk menyertakan kondisi pencarian
    $query = "SELECT * FROM pengadaan WHERE tanggal LIKE '%$key%' OR judul LIKE '%$key%' OR asal LIKE '%$key%' OR jumlah LIKE '%$key%'  OR keterangan LIKE '%$key%'";
    $datas = mysqli_query($koneksi, $query) or die(mysqli_error($koneksi));
} else {
    // Jika formulir tidak dikirimkan, ambil semua data
    $datas = mysqli_query($koneksi, "SELECT * FROM pengadaan") or die(mysqli_error($koneksi));
}
...
<div class="container-fluid">
                ...
                <div class="card-body">
                    <form action="" method="post">
                        <div class="row mb-3">
                            ...
                            <a href="tambahpengadaan.php" class="btn btn-primary mb-4"><i class="bi bi-bookmark-plus-fill">TAMBAH DATA</i>
   ...
   // Hapus data dari database
        $query = "DELETE FROM pengadaan WHERE id = ?";
...
// Redirect ke halaman utama setelah penghapusan
                            ...
                            ?>
                                    <tr style="background-color: <?= $rowColor; ?>">
                                        ...
                                        <td>
                                            <a href="editpengadaan.php?id=<?= $row['id']; ?>" class="btn btn-sm btn-warning">Edit</a>
